...
> **适用场景：** 通过 Kong 网关的 access log 中 `rt`、`uct`、`uht`、`urt` 四个时间指标，快速定位性能瓶颈归属——是网络、上游服务、还是 Kong 自身。
...
| 指标 | 变量 | 衡量什么 | 飙高口诀 | 颜色 |
|:----:|:----:|:--------|:--------|:----:|
| **rt** | `$request_time` | Kong 进程内部总耗时 | `rt - urt` 高 → 🟡 Kong 自身 | 🟢 |
| **uct** | `$upstream_connect_time` | Kong → 上游 TCP 建连（含 DNS + SSL） | `uct` 高 → 🔴 网络建连 | 🔴 |
| **uht** | `$upstream_header_time` | 建连 + 发请求 + 等上游首字节 | `uht - uct` 高 → 🔵 上游服务慢 | 🔵 |
| **urt** | `$upstream_response_time` | 建连 + 发请求 + 收完完整响应体 | `urt - uht` 高 → 🔴 传 body 慢 | 🟠 |

> ⚠️ **重要：** `rt` 不包含「客户端 → Kong」的网络传输时间，它是 Kong 进程内部计时。

---

## 📑 目录

- [一、四个时间指标的定义](#一四个时间指标的定义)
- [二、五段式耗时拆解](#二五段式耗时拆解)
- [三、推荐日志格式](#三推荐日志格式)
- [四、故障定位全景](#四故障定位全景)
- [五、完整判断口诀](#五完整判断口诀)
- [六、uct 高的常见原因](#六uct-高的常见原因)
- [七、生产日志分析脚本](#七生产日志分析脚本)
- [八、状态码辅助判断](#八状态码辅助判断)
- [九、实战排查案例](#九实战排查案例)
- [十、排查流程总结](#十排查流程总结)
...
> **一句话导读：** 理解四个指标的时间起点、终点和包含关系，是后续所有排查的基础。

### 时间轴全景

```
时间轴 -->  Kong前置  建连   发请求  等首字节  --- 接收body ---  Kong后置  发响应
              |         |      |       |         |                |         |
...
推导公式：
  urt - uct = 发请求 + 上游处理 + 传响应体（上游交互总耗时，不含建连）
  urt - uht = 上游发送响应体的耗时（网络传输 body 的时间）
  uht - uct = 上游服务处理时间（收到请求 --> 返回首字节）
  urt - uct = (uht - uct) + (urt - uht)（即上面两项之和）
```
...
| 段 | 计算公式 | 衡量 | 正常基线 |
|:--:|:--------|:-----|:--------|
| ① Kong 前置 | `rt - urt` 的一部分 | 插件 before 阶段 | < 5ms |
| ② 网络建连 | `uct` | Kong <-> 上游 TCP 质量 | < 5ms（同机房） |
| ③ 上游处理 | `uht - uct` | 上游服务计算耗时 | 视业务而定 |
| ④ 传响应体 | `urt - uht` | 上游 --> Kong 的带宽/网络 | 通常 < 10ms |
| ⑤ Kong 后置 | `rt - urt` | 插件 after + 发给客户端 | < 10ms |
...
> **一句话导读：** 一个涵盖四个指标的推荐 Nginx 日志格式，开箱即用。
...
> **一句话导读：** 六种典型故障场景的日志特征和排查方法，对号入座即可。
...
|:--:|:----:|:----:|
| ② 网络建连 | `uct = 2ms` | ✅ 正常 |
| ③ 上游处理 | `uht - uct = 48ms` | ✅ 正常 |
| ④ 传响应体 | `urt - uht = 2ms` | ✅ 小 body |
| ①+⑤ Kong 开销 | `rt - urt = 13ms` | ✅ 正常 |
...
| ④ 传响应体 | `urt - uht = 5ms` | ✅ 正常 |
| ①+⑤ Kong 开销 | `rt - urt = 505ms` | ⚠️ 主要是在等上游关闭连接 |
...
| ①+⑤ Kong 开销 | `rt - urt = 100ms` | ✅ 正常 |

> 🎯 **口诀：`urt - uht` 飙高 → 上游到 Kong 的网络带宽/质量问题**

> ⚠️ **这是没有 `urt` 就看不出来的问题！** 只有 `uht` 时，你只知道「上游 60ms 就返回了首字节」，以为一切正常，但实际响应体传了 3 秒。

**排查命令：**

```bash
# 测到上游的带宽
iperf3 -c upstream-host

# 大包测试，检查 MTU 问题
ping -s 1400 upstream-host
```
...
|:--:|:----:|:-----|
| ② 网络建连 | `uct = 3ms` | ✅ 正常 |
| ③ 上游处理 | `uht - uct = 47ms` | ✅ 正常 |
| ④ 传响应体 | `urt - uht = 10ms` | ✅ 正常 |
| ①+⑤ Kong 开销 | **`rt - urt = 3s`** | 🟡 **Kong 自己花了 3 秒** |
...
| ①+⑤ Kong 开销 | `rt - urt = 1.01s` | 🟡 Kong 也有开销 |
...
> **一句话导读：** `uct` 是最容易让人困惑的指标，本章深入拆解六种常见原因，重点讲解「丢包导致整数秒跳变」这一经典模式。

### 原因总览

| 原因 | 特征 | 常见场景 | 解决方案 |
|:-----|:-----|:---------|:--------|
| DNS 解析慢 | 首次请求 uct 高，后续正常 | Kong 配置的 DNS TTL 过短、DNS 服务器响应慢 | 配置 `dns_resolver`，启用 `dns_cache` |
| 上游 TCP 队列满 | 高并发时 uct 飙升 | 上游服务连接数打满、`somaxconn` 配置过小 | 调大上游 `somaxconn`，检查 backlog |
| 跨区域/跨机房 | 所有请求 uct 都高 | Kong 和上游部署在不同机房/可用区 | 同机房部署，减少网络跳数 |
| SSL 握手慢 | HTTPS 上游 uct 比 HTTP 高 50ms+ | 证书链过长、未启用 session reuse | 启用 session reuse，检查证书链长度 |
| Kong keepalive 池耗尽 | 突发时 uct 从 1ms 跳到 100ms+ | QPS 突增、连接池配置过小 | 调大 `upstream_keepalive` |
| **网络丢包** | **1s / 3s / 7s 整数秒跳变** | **链路不稳定、网卡队列溢出、中间设备故障** | 见下方详细分析 |
...
丢SYN：  SYN --X  (等1s重传) --> SYN --> SYN-ACK --> ACK
         uct ≈ 1002ms   (多了 1s 等待)

丢SYN-ACK： SYN --> SYN-ACK X (等1s) --> SYN-ACK --> ACK
            uct ≈ 1002ms
```
...
> **一句话导读：** 两个开箱即用的 awk 脚本——实时打标签 + 离线算分位数。
...
> **一句话导读：** 结合 HTTP 状态码和 upstream_status，可以进一步缩小问题范围。
...
> **一句话导读：** 六个真实生产案例，覆盖网络、服务、Kong、DNS、连接池、流式输出等典型场景。每个案例按 **现象 → 分层分析 → 定位 → 解决** 四步走。
...
| 上游处理 | 60ms | ✅ 上游确实没问题 |
| 传 body | 10ms | ✅ |
| **Kong 开销** | **720ms** | 🟡 **Kong 自身开销 P95 = 720ms！** |
...
> **一句话导读：** 面对性能问题的标准化操作步骤——分层统计 → 定位最大层 → 按层排查。
...
> 📌 **最后提醒：** 四个指标 + 五段拆解 + 六种场景 = 覆盖 95% 的 Kong 网关性能问题。遇到问题时，先跑分层统计脚本，再按口诀对号入座，最后用排查命令验证。
